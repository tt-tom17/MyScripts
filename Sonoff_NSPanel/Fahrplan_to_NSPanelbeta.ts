...
async function JSON_Umwandeln(JSON_Plan: string, Haltestelle: string) {
    try {
        ...
        if (parseInt(h) > AnzahlHaltestellen) {
            ...
            if (Debug) log('Anzahl der Abfahrten Route ' + (h) + ': ' + HaltestellenPlan.journeys.length);

            for (let i = 0; i < HaltestellenPlan.journeys.length; i++) {

                if (i == AnzahlAbfahrten) {
                    console.warn('Es werden nur die ersten ' + AnzahlAbfahrten + ' Abfahrten pro Route eingelesen');
                    break;
                };

                if (Debug) console.log('Beginn Auswertung Abfahrt: ' + i);

                let Abfahrt = HaltestellenPlan.journeys[i].legs[0];

                //if (Debug) console.log(JSON.stringify(Abfahrt));

                AktuelleAbfahrzeit = getAttr(Abfahrt, 'departure');
                GeplanteAbfahrzeit = getAttr(Abfahrt, 'plannedDeparture')
                ...
                Timedelay = getAttr(Abfahrt, 'departureDelay');
                ...
                if (Debug) log('Popup öffnen: ' + VerspaetungPopup + ', Verspätung in Minuten: ' + Minuten + ' Popuptext: ' + Notifytext);
