 ...
 */
const NummernHaltestellen: string[] = ["0", "2"]; // Anzahl der Haltestellen / Anzeigetafeln
 ...
 */
const NummernRouten: string[] = ["1"]; // Anzahl der Routen
...
async function json_HaltestelleUmwandeln(json_Haltestelle: string, haltestelle: string) {
  try {
    ...
    for (let i = 0; i < haltestellenPlan.length; i++) {
      ...
      if (Debug)
        log(
          `Abfahrzeit geplant: ${geplanteAbfahrzeit} Richtung: ${richtung} Fahrzeug: ${fahrzeugTyp} Verspätung in sec: ${timeDelay} aktuelle Abfahrzeit: ${aktuelleAbfahrzeit}`
        );
