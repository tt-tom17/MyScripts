 ...
 */
const NummernHaltestellen: string[] = ['0', '2']; // Auswahl der Haltestellen / Anzeigetafeln
const anzahlAbfahrtenHaltestelle: number = 6; // Abfahrten an der Haltestelle -> mehr als 6 kann das NSPanel noch nicht anzeigen
 ...
 */
const NummernRouten: string[] = ['1']; // Auswahl der Routen
const anzahlAbfahrtenRoute: number = 3; // Abfahrten an der Route -> mehr als 3 kann das NSPanel noch nicht anzeigen
...
async function init_Datenpunkte() {
    try {
            ...
            for (let i = 0; i < anzahlAbfahrtenHaltestelle; i++) {
                if (existsObject(dp_Userdata + 'FahrplanAnzeiger.Haltestelle' + haltestelle + '.Abfahrt' + String(i)) == false) {
                    ...
                    setObject(dp_Alias + 'FahrplanAnzeiger.Haltestelle' + haltestelle, {
                        type: 'device',
                        common: { role: 'timeTable', name: { de: 'Haltestelle ' + haltestelle, en: 'Station ' + haltestelle } },
                        native: {},
                    });
                    setObject(dp_Alias + 'FahrplanAnzeiger.Haltestelle' + haltestelle + '.Abfahrt' + String(i), {
                        type: 'channel',
                        common: { role: 'timeTable', name: { de: 'Abfahrt ' + String(i), en: 'Departure ' + String(i) } },
                        native: {},
                    });
        ...
        for (let h = 0; h < NummernRouten.length; h++) {
            let route: string = NummernRouten[h];
 ...
 */
on(/^fahrplan\.0+\.DepartureTimetable[0-9]+\.JSON/, (obj) => {
    if (obj.id) {
        let Haltestellennummer: string = obj.id.substring(29, obj.id.length - 5);

        log(`Trigger ausgelöst durch ${obj.id} Haltestellennummer: ${Haltestellennummer}`, `info`);
        if (NummernHaltestellen.includes(Haltestellennummer)) {
            json_HaltestelleUmwandeln(obj.id, Haltestellennummer);
        } else {
            log(`Haltestellennummer: ${Haltestellennummer} wird nicht ausgewertet`, `info`);
        }
    } else {
        log(`keine id übergeben`, `warn`);
    }
});
 ...
 *  fahrplan.0.X.JSON
 */
on(/^fahrplan\.0+\.[0-9]+\.JSON/, (obj) => {
    if (obj.id) {
        let Route: string = obj.id.substring(11, obj.id.length - 5);

        log(`Trigger ausgelöst durch ${obj.id} Route: ${Route}`, `info`);
        if (NummernRouten.includes(Route)) {
            json_RouteUmwandeln(obj.id, Route);
        } else {
            log(`Route: ${Route} wird nicht ausgewertet`, `info`);
        }
    } else {
        log(`keine id übergeben`, `warn`);
    }
